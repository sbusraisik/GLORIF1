...
# Calculate IQR and filter out outliers from both datasets
cat("Removing outliers...\n")
upper_bound <- 10000

filtered_data <- plot_data %>%
  filter(Observed_Mean <= upper_bound) %>%
  filter(Modeled_Mean <= upper_bound)
cat("Outliers removed.\n")

# Create a scatter plot with different colors for the datasets
cat("Creating scatter plot...\n")
plot <- ggplot(filtered_data, aes(x = Observed_Mean, y = Modeled_Mean)) +
  ...
  labs(title = "Observed vs Modeled Mean Streamflow (Filtered Data)",
  ...
  scale_y_log10()    # Logarithmic scale for y-axis # Center the plot title
...
# Save the plot
cat("Saving plot...\n")
ggsave(file.path(output_dir, "obs_vs_model_mean_streamflow_GSIM_filtered_logscale_units.pdf"), plot, width = 8, height = 6)
cat("Plot saved to", file.path(output_dir, "obs_vs_model_mean_streamflow_GSIM_filtered_logscale_units.pdf"), "\n")
